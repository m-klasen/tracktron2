...
__all__ = ["load_coco_json", "load_coco_json_vid", "load_sem_seg", "convert_to_coco_json", "COCO_vid"]
...
def load_coco_json_vid(json_file, image_root, dataset_name=None, extra_annotation_keys=None):
    """
    Load a json file with COCO's instances annotation format.
    Currently supports instance detection, instance segmentation,
    and person keypoints annotations.
    ...
    Notes:
        1. This function does not read the image files.
           The results do not have the "image" field.
    """
    def sample_ref(idx, vid_infos, img_ids):
        # sample another frame in the same sequence as reference
        vid, frame_id = idx
        vid_info = vid_infos[vid]
        ref_idx = (vid, (frame_id-1))
        if (frame_id-1) != frame_id and ref_idx in img_ids:
            return ref_idx
        else:
            ...
            for i in sample_range:
                ...
                ref_idx = (vid, i)
                if i != frame_id and ref_idx in img_ids:
                    valid_samples.append(ref_idx)
            assert len(valid_samples) > 0
            return random.choice(valid_samples)

    def _parse_ann_info( ann_info, frame_id, with_mask=True):
        ...
        for i, ann in enumerate(ann_info):
            ...
            if ann['iscrowd']:
                ...
                gt_classes.append(cat2label[ann['category_id']])
            if with_mask:
                gt_segmentation.append(mask_util.frPyObjects(segm, segm.get('size')[0], segm.get('size')[1]))
                gt_masks.append(coco_api.annToMask(ann, frame_id))
                mask_polys = [
    ...
    if dataset_name is not None:
        ...
        vid_ids = coco_api.getVidIds()
        vid_infos = []
        for i in vid_ids:
            info = coco_api.loadVids([i])[0]
    ...
    ann_keys = ["iscrowd", "bboxes", "keypoints", "category_id", "segmentations", "id"] + (extra_annotation_keys or [])

    num_instances_without_valid_segmentation = 0

    for (vid, frame_id) in img_ids:
        idx = (vid,frame_id)
        vid_info = vid_infos[vid]
        record = {}
        record["file_name"] = os.path.join(image_root, vid_info["filenames"][frame_id])
        _, ref_frame_id = sample_ref(idx,vid_infos,img_ids)
        record["ref_filename"] = os.path.join(image_root, vid_info["filenames"][ref_frame_id])

        vid_id = vid_infos[vid]['id']
        ann_ids = coco_api.getAnnIds(vidIds=[vid_id])
        ann_info = coco_api.loadAnns(ann_ids)
        #ann = _parse_ann_info(ann_info, frame_id)
        #ref_ann = _parse_ann_info(ann_info, ref_frame_id)

        record["height"] = vid_info["height"]
        record["width"] = vid_info["width"]
        record["ref_image_id"] = ref_frame_id
        ...
        # assert ann_info[0]["id"] == frame_id
        # assert ann_info[1]["id"] == ref_frame_id

        #assert anno.get("ignore", 0) == 0, '"ignore" in COCO json file is not supported.'
        
            obj = dict()
            if "iscrowd" in anno: obj["iscrowd"] = anno["iscrowd"]
            if "bboxes" in anno: 
                obj["bbox"] = anno["bboxes"][frame_id]
                if "category_id" in anno: 
                    obj["category_id"] = anno["category_id"]
                    obj["ref_category_id"] = anno["category_id"]
            if "bboxes" in anno: obj["ref_bbox"] = anno["bboxes"][ref_frame_id]
            if anno["bboxes"][frame_id] is None: continue
            if anno["bboxes"][ref_frame_id] is None: continue
            ...
            if segm:  # either list[list[float]] or dict(RLE)
                ...
                segm = mask_util.frPyObjects(segm, segm.get('size')[0], segm.get('size')[1])
                obj["segmentation"] = segm
        ...
        if 'segmentations' in anns[0]:
            ...
            for id, ann in enumerate(anns):
                ...
                for seg in ann['segmentations']:
                    ...
                    if seg:
                        ann['areas'].append(mask_util.area(seg))
                        if len(ann['bboxes']) < len(ann['areas']):
                            ann['bboxes'].append(mask_util.toBbox(seg))
                    else:
