...
def build_detection_fixmatch_loader(cfg, mapper=None):
    """
    A data loader is created by the following steps:
    ...
    if mapper is None:
        mapper = DatasetMapper(cfg, True)
    dataset = MapDataset(dataset, mapper)
    dataset_labeled, dataset_unlabeled = torch.utils.data.random_split(
            dataset, [int(len(dataset)*0.2), int(len(dataset)*0.8)]
            )
    sampler_name = cfg.DATALOADER.SAMPLER_TRAIN
    logger = logging.getLogger(__name__)
    logger.info("Using training sampler {}".format(sampler_name))
    if sampler_name == "TrainingSampler":
        sampler_l = samplers.TrainingSampler(len(dataset_labeled))
        sampler_u = samplers.TrainingSampler(len(dataset_unlabeled))
    elif sampler_name == "RepeatFactorTrainingSampler":
        sampler = samplers.RepeatFactorTrainingSampler(
            dataset_dicts, cfg.DATALOADER.REPEAT_THRESHOLD
        )
    ...
    if cfg.DATALOADER.ASPECT_RATIO_GROUPING:
        ...
        data_loader_labeled = AspectRatioGroupedDataset(data_loader_labeled, images_per_worker)
        data_loader_unlabeled = AspectRatioGroupedDataset(data_loader_unlabeled, images_per_worker)
    else:
        batch_sampler = torch.utils.data.sampler.BatchSampler(
            sampler, images_per_worker, drop_last=True
        )
