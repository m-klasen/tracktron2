...
def read_rgbd_image(file_name, format=None):
    """
    Read an image into the given format.
    Will apply rotation and flipping if the image has such exif information.
    ...
    image = np.asarray(image)
    rgb = image[:, : image.shape[1] // 2, :]
    depth_packed = image[:, (image.shape[1] // 2) :, :]
    depth = (depth_packed[:, :, 0] * 2 ** 8 + depth_packed[:, :, 1]).astype(
        np.uint8
    )
...
def transform_instance_annotations(
    ...
    bbox = BoxMode.convert(annotation["ref_bbox"], annotation["bbox_mode"], BoxMode.XYXY_ABS)
    annotation["ref_bbox"] = transforms.apply_box([bbox])[0]
...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    ref_boxes = [BoxMode.convert(obj["ref_bbox"], obj["bbox_mode"], BoxMode.XYXY_ABS) for obj in annos]
    ref_target = Instances(image_size)
    ref_boxes = ref_target.gt_boxes = Boxes(ref_boxes)
    ref_boxes.clip(image_size)

    ref_classes = [obj["ref_category_id"] for obj in annos]
    ref_classes = torch.tensor(ref_classes, dtype=torch.int64)
    ref_target.gt_classes = ref_classes
...
def ref_annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    ref_classes = [obj["gt_pids"] for obj in annos]
    ref_classes = torch.tensor(ref_classes, dtype=torch.int64)
    ref_target.gt_classes = ref_classes
    return ref_target
