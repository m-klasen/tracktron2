...
class Res5ROIHeads(ROIHeads):
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        features,   ref_features     = features
        proposals,  ref_proposals    = proposals
        targets,    ref_targets      = targets
...
class StandardROIHeads(ROIHeads):
    ...
    def _init_box_head(self, cfg, input_shape):
        ...
        self.track_head = build_track_head(cfg, ShapeSpec(channels=in_channels, height=pooler_resolution, width=pooler_resolution) )
        if self.track_head:
    ...
    def forward(
        ...
        if self.training:
            assert targets
            proposals = self.label_and_sample_proposals(proposals, targets)
            #ref_proposals = self.label_and_sample_proposals(ref_proposals, ref_targets)
        #del targets

        if self.training:
            box_features, losses = self._forward_box(features, proposals)
            #ref_box_features, _ = self._forward_box(ref_features, ref_proposals)
            ...
            ref_features = [ref_features[f] for f in self.in_features]
            ref_boxes = [x.gt_boxes for x in ref_targets]
            ref_box_features = self.box_pooler(ref_features, ref_boxes)

            x_n = [len(x) for x in proposals]
            ref_x_n = [len(x) for x in ref_boxes]
            match_score = self.track_head(box_features,ref_box_features,x_n,ref_x_n)
            proposal_classes = [x.gt_classes for x in proposals]

            


            proposal_classes = torch.cat(proposal_classes)
            weights = torch.zeros((proposal_classes.size())).cuda()
            ones = torch.ones((proposal_classes.size())).cuda()
            weights = torch.where(proposal_classes == 80,weights,ones)
            track_loss = self.track_head.loss(match_score,proposal_classes,weights)
